...
### 3.4 Resolution Tree Engine (Backtracking)
**Goal**: Enforce strict hierarchical logic for issue resolution.

*   **Logic Model**: Connection-Aware Directed Acyclic Graph (DAG).
*   **Metadata**: Nodes store `connection_type` (`MAIN` | `SIDE_LEFT` | `SIDE_RIGHT`) instead of inferred geometric positions.
*   **Backtracking Algorithm**:
    *   **Path Discovery**: The system resolves a node's full lineage by backtracking through `parent_node_id` links.
    *   **Branch Origin**: A "Side Branch" is discovered when a backtracking algorithm hits a node with `connection_type == 'SIDE_...'`.
    *   **Blue Lock**: Side branches are gated from promotion to **Blue** if the corresponding point on the **Main** branch path is not yet promoted.
*   **The Red Axe**: Recursive deletion logic that uses parent-child links to truncate entire logical branches while protecting the Main path.
